...
namespace Ridebury
{
    ...
    /// </summary>
    public class RideburyGame : MonoBehaviour
    {
        ...
        // within the first ~3 minutes instead of hiding the first one at L8 — L1 normal, then EVERY bonus type
        // back to back (L2-L5), so a reviewer sees the whole game in one short sitting. The teaching levels were
        // pushed BEHIND the showcase and MUST stay None: L1 runs the core-loop tutorial, L6 the bus-capacity +
        // RECOLOR joker coach (a forced-joker step cannot run under a countdown), L7 the minivan + diagonals banners.
        static readonly BonusKind[] Showcase =
        {
            ...
            BonusKind.MysteryRush,   // L3
            BonusKind.CoinRush,      // L4
            BonusKind.TimeAttack,    // L5 completes the set
            BonusKind.None,          // L6 authored — bus capacity + RECOLOR joker tutorial
            BonusKind.None,          // L7 first procedural — minivans + diagonals
        };
        ...
        /// no vehicle type the player has not met yet) because no joker is unlocked before L6.</summary>
        ...
        // Model per queue slot: every consecutive run of one colour shares ONE character, so a run of
        // 3 blues reads as three identical figures. Parallel to `groups`, built once per level.
        readonly List<GameObject> runPrefabs = new List<GameObject>();
        int nextGroupIndex;
        ...
        void StreamNext()
        {
            ...
            {
                int gi = nextGroupIndex++;
                var u = CreateUnit(groups[gi], gi);
        ...
        void StartLevel(int levelNumber)
        {
            ...
            // Mystery Rush inside the prologue stays CARS ONLY: MixForLevel would hand it minivans early, and the
            // authored prologue boards (no minivans through L6) keep the minivan reveal for L7's banner.
            ...
            // so a prior level's joker/diagonal coach can't bleed into this one (e.g. Lv6's RECOLOR ring lingering
            // into Lv7 when jumping levels). Each branch below re-arms its own banner/pointer fresh.
            ...
            else if (levelNumber == 6 && !SaveSystem.FreeJokerGranted) // FIRST Lv6 visit only — skip on every replay (FreeJokerGranted persists once the RECOLOR unlock coach has run, mirroring level 1's TutorialDone gate)
                StartCoroutine(ShowBanner("Buses seat 10 people!", 3.5f, StartJokerTutorial)); // teach bus capacity, then the RECOLOR joker
            else if (levelNumber == 7)
            {   // Level 7 = first PROCEDURAL level = the first with minivans (L1-6 are authored boards / showcase
                // bonus rounds; LevelGenerator.MixForLevel only applies from 7 up). One-time capacity note FIRST,
                // then the (every-visit) diagonal banner chains after it so the two never overlap.
                if (!SaveSystem.MinivanTipShown)
                {
                    SaveSystem.MinivanTipShown = true; // set immediately: a retry/replay of Lv7 must not re-show it
                    StartCoroutine(ShowBanner("Minivans seat 6 people!", 3.5f,
                        () => StartCoroutine(ShowBanner("New: vehicles can now move DIAGONALLY!", 5f))));
                }
                else
                    StartCoroutine(ShowBanner("New: vehicles can now move DIAGONALLY!", 5f));   // diagonals unlock at level 7
            }
        ...
        void StartJokerTutorial() // legacy Lv6 entry, chained 3.5s after the bus-capacity banner
        {
            // RECOLOR coaching belongs ONLY to its unlock level (Lv6). The capacity banner defers this by 3.5s, so
        ...
        // Paint a catalog model's colour zone to `colorMat`. ONLY the slot named "Tint" is replaced — on the
        // Ridebury people that is a cap covering the top ~20-25% of the body, so the character keeps its own
        // colours and stays recognisable while still carrying the round colour where it reads best (the top
        // of the silhouette). Painting the whole body instead makes every character an identical blob.
        //
        // Falls back to the old "everything that isn't a face slot" rule for any model with no Tint slot, so
        // a differently-authored character pack still works.
        //
        // Returns the tinted renderer (null if the model has neither renderer), and the FIRST tinted slot in
        // `bodyIndex` — the slot LineUnit.Reveal swaps back on a mystery reveal. That is exact here because
        // the export gives every character exactly one Tint slot.
        static Renderer TintModelBody(GameObject model, Material colorMat, out int bodyIndex)
        {
            ...
            for (int i = 0; i < mats.Length; i++)
                if (mats[i] != null && mats[i].name.ToLowerInvariant().Contains("tint")) { hasTint = true; break; }

            for (int i = 0; i < mats.Length; i++)
            {
                string n = mats[i] != null ? mats[i].name.ToLowerInvariant() : string.Empty;
                bool paint = hasTint ? n.Contains("tint") : !n.Contains("face");
                if (!paint) continue;
                mats[i] = colorMat;
        ...
        LineUnit CreateUnit(LineGroup g, int queueIndex)
        {
            var go = new GameObject("Person");
            go.transform.SetParent(boardRoot, false);
            var u = go.AddComponent<LineUnit>();
            u.color = g.color; u.golden = g.golden; u.mystery = g.mystery;
            GameObject prefab = (queueIndex >= 0 && queueIndex < runPrefabs.Count) ? runPrefabs[queueIndex] : null;
            BuildPersonVisual(u, go.transform, g.color, g.golden, g.mystery, prefab);
            OutlineAll(go); // toon ink edge on the boarding person (imported skinned OR code fallback)
            return u;
        }

        // One character per COLOUR RUN. Every consecutive stretch of the same colour gets a single model,
        // so "2 red, 3 blue, 3 red" reads as two identical reds, then three identical blues, then three
        // identical reds. Neighbouring runs are never given the SAME model, otherwise the boundary between
        // two runs would be invisible and the grouping would do nothing. A later run of a colour may reuse
        // an earlier run's model — the rule is per run, not per colour.
        void BuildRunPrefabs()
        {
            runPrefabs.Clear();
            if (groups == null) return;
            GameObject run = null;        // model of the run in progress (null = start a fresh one)
            GameObject last = null;       // model handed to the previous queue slot
            GameObject beforeGap = null;  // model of the run a mystery interrupted
            for (int i = 0; i < groups.Count; i++)
            {
                if (groups[i].mystery)
                {
                    // A mystery hides its colour, so it must NOT wear its run's model — that would name
                    // the colour outright. It also BREAKS the run: if the runs either side kept the same
                    // model, a grey figure sandwiched between two identical characters would give the
                    // colour away by position alone. So the run after a mystery also starts fresh.
                    var m = PickRunPrefab(last, run);
                    runPrefabs.Add(m);
                    beforeGap = last;   // the model actually STANDING before the mystery — not the run's,
                                        // which is stale when two mysteries sit next to each other
                    last = m; run = null;
                    continue;
                }
                if (run == null || groups[i].color != groups[i - 1].color)
                {
                    run = PickRunPrefab(last, beforeGap);
                    beforeGap = null;
                }
                runPrefabs.Add(run);
                last = run;
        ...
        // A random catalog model that is neither `avoidA` nor `avoidB`, so adjacent runs stay distinct and
        // a mystery never matches the run it sits in (or the run resuming after it).
        GameObject PickRunPrefab(GameObject avoidA, GameObject avoidB)
        {
            if (peopleCatalog == null || !peopleCatalog.HasModels) return null;
            int n = peopleCatalog.prefabs != null ? peopleCatalog.prefabs.Length : 0;
            bool canAvoid = n > 2;   // needs room to dodge both and still have a choice
            for (int tries = 0; tries < 12; tries++)
            {
                var p = peopleCatalog.RandomPrefab();
                if (p == null) continue;
                if (!canAvoid || (p != avoidA && p != avoidB)) return p;
            }
            return peopleCatalog.RandomPrefab();
        }
        ...
        {
            // `prefab` is this person's colour-run model; fall back to a random one (crowd/no run table).
            if (prefab == null && peopleCatalog != null) prefab = peopleCatalog.RandomPrefab();
            if (prefab == null)
            {
