...
namespace BusJam
{
    /// <summary>Import rules for the BusJam queue characters. They are static one-mesh figures (~2.1k tris)
    ...
    public class BusJamPeopleImporter : AssetPostprocessor
    {
        const string Folder    = "Assets/Characters/BusJamPeople/";
        const string MatFolder = "Assets/Characters/BusJamPeople/Materials";
    ...
    /// <summary>"BusJam ▸ Diagnose People" — prints what the queue characters ACTUALLY resolve to
    ...
    {
        [MenuItem("BusJam/Diagnose People")]
        public static void Diagnose()
        {
            var cat = Resources.Load<PeopleCatalog>("PeopleCatalog");
            if (cat == null) { Debug.LogError("[BusJam] No PeopleCatalog in Resources."); return; }
            Debug.Log($"[BusJam] catalog: {(cat.prefabs?.Length ?? 0)} entries, HasModels={cat.HasModels}, " +
            ...
            {
                var p = cat.prefabs[i];
                if (p == null) { Debug.LogError($"[BusJam] entry {i} is a MISSING reference"); bad++; continue; }
                ...
                if (r == null) { Debug.LogError($"[BusJam] {p.name}: NO renderer"); bad++; continue; }
                ...
                string msg = $"[BusJam] {p.name}: {r.GetType().Name} enabled={r.enabled} " +
            ...
            Debug.Log($"[BusJam] Diagnose People finished — {bad} problem(s).");
        }
    }
}
